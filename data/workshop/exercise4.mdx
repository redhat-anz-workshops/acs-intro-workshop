...
Red Hat Advanced Cluster Security for Kubernetes (RHACS) uses an extended Berkeley Packet Filter (eBPF) probe to detect processes running in containers, and also detect container network traffic flows. The eBPF probe runs on every Red Hat Enterprise Linux CoreOS (RHCOS) node of an OpenShift cluster.
